...
This README is the **System Architecture Document** deliverable. It is written to be read alongside the rest of the submission, and each required deliverable is called out explicitly below so an evaluator can locate it in one pass.
...
| — | Presentation slides (provided template) | Submitted alongside the repo link |

---

## Requirement Traceability

Mapping the problem statement's **Technical Core Requirements** to what this repository implements, so the connection is explicit rather than implied.
...
These three areas are called out explicitly because the assessment brief asks the architecture document to cover them directly.
...
Stated directly rather than glossed over, since the problem statement rewards transparency about tool-calling architecture and self-correction as much as raw results:
...
- [ ] Quantitative savings dashboard (`dashboard.png`, `summary.json`) — generate from the final 14-day run
- [x] System Architecture Document — this README
- [ ] PoC demonstration video (max 3 minutes)
- [ ] Presentation slides (provided template, filled in)
